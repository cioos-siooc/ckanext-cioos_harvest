"""
CIOOS CKAN Schema Harvester.

A harvester for CKAN instances that don't have a matching schema.
This harvester maps remote fields to CIOOS schema fields.
"""
...
class CIOOSCKANSchemaHarvester(HarvesterBase):
    """
    A Harvester for CKAN instances without matching schema.

    This harvester maps remote CKAN fields to CIOOS schema fields,
    handling multilingual fields, keywords, and other CIOOS-specific
    ...
    def _get_content(self, url, params=None):
        ...
        try:
            ...
            raise ContentFetchError("HTTP timeout: %s" % str(e))
        except requests.exceptions.TooManyRedirects as e:
            raise ContentFetchError("HTTP too many redirects: %s" % str(e))
        except requests.exceptions.RequestException as e:
            raise ContentFetchError("HTTP request exception: %s" % str(e))
    ...
    def _get_organization(self, base_url, org_name):
        ...
        try:
            ...
            log.debug("Could not fetch/decode remote organization")
    ...
    def gather_stage(self, harvest_job):
        log.debug(
            "In CIOOSCKANSchemaHarvester gather_stage (%s)", harvest_job.source.url
        )
    ...
    def _search_for_datasets(self, remote_ckan_base_url, fq_terms=None):
        ...
        while True:
            ...
            try:
                ...
                raise SearchError(
                    "Response JSON did not contain result/results: %r" % response_dict
                )
    ...
    def import_stage(self, harvest_object):
        log.debug("In CIOOSCKANSchemaHarvester import_stage")
        ...
        try:
            ...
            package_type = self.config.get(
                "force_package_type",
                package_dict.get("type", package_dict.get("resource-type")),
            )
            ...
            if not package_dict.get("metadata-language"):
                package_dict["metadata-language"] = package_dict.get(
            ...
            if not package_dict.get("cited-responsible-party"):
                ...
                for x in package_dict.get("contacts", []):
                    ...
                    if x.get("organization"):
                        try:
                            org = self._get_organization(
                                harvest_object.source.url, x.get("organization")
                            )
                        except RemoteResourceError:
                            log.warning(
                                "Could not fetch organization %s for contact",
                                x.get("organization"),
                            )
                    crp.append(
                        {
            ...
            if not package_dict.get("progress"):
                package_dict["progress"] = package_dict.get(
