...
class DatastreamSitemapHarvester(WAFHarvesterISO19115_3):
    ...
    def translate_string(
        self, redis_conn, string_to_translate, source_lang="en", target_lang="fr"
    ):
        store_name = "%s_%s_to_%s" % (
            self.redis_translation_store,
            source_lang,
            target_lang,
        )
        ...
        try:
            ...
            aws_trans_obj = translate.translate_text(
            ...
            result.append(
                {
                    ...
                    "contact-info_online-resource": contact_info.get(
                        "online-resource", ""
                    ),
                    "role": org.get("role", ""),
                }
    ...
    def gather_stage(self, harvest_job, collection_package_id=None):
        ...
        try:
            for url_node in sitemap_tree.findall(
                ...
                last_modified_node = url_node.find(
        ...
        for item in possible_changes:
            if (
                not url_to_modified_harvest[item]
        ...
        def create_extras(url, date, status):
            ...
            if collection_package_id:
                extras.append(
                    HOExtra(key="collection_package_id", value=collection_package_id)
                )
            return extras
        ...
        for location in new:
            ...
            obj = HarvestObject(
                ...
                extras=create_extras(
                    location, url_to_modified_harvest[location], "new"
                ),
                guid=guid,
            )
            obj.save()
        ...
        for location in change:
            obj = HarvestObject(
                job=harvest_job,
                extras=create_extras(
                    location, url_to_modified_harvest[location], "change"
                ),
