...
def _parse_gml_to_shapely(gml_string):
    ...
    try:
        content = (
            gml_string.encode("utf-8") if isinstance(gml_string, str) else gml_string
        )
...
def _iso_date_time_to_utc(value):
    ...
    if re.search(r"[+-]\d{4}", value):
        ...
        timedelta = datetime.timedelta(
            hours=int(value[-5:][1:3]), minutes=int(value[-5:][-2:])
    ...
    try:
        ...
        try:
            utc_dt = (
                datetime.datetime.strptime(value[:post_remove], "%Y-%m-%dT%H:%M:%S")
                + timedelta
            )
        except Exception:
            utc_dt = (
                datetime.datetime.strptime(value[:post_remove], "%Y-%m-%dT%H:%M:%S.%f")
                + timedelta
            )
...
def _unescape_unicode(encoded_str):
    ...
    while re.search(r"\\u[0-9a-fA-F]{4}", encoded_str):
        if isinstance(encoded_str, str):
            encoded_str = encoded_str.encode("raw_unicode_escape").decode(
                "unicode_escape"
            )
        else:
            encoded_str = (
...
def _parse_responsible_party_cioos(el):
    ...
    return {
        ...
        "organisation-name": _text(
            el, "cit:party/cit:CI_Organisation/cit:name/gco:CharacterString/text()"
        ),
        "contact-info_email": _first_text(
            el,
        ...
        "contact-info_online-resource": _first_text(
            el,
        ...
        "individual-uri_code": _first_text(
            el,
        ...
        "individual-uri_authority": _first_text(
            el,
        ...
        "individual-uri_code-space": _first_text(
            el,
        ...
        "individual-uri_version": _first_text(
            el,
...
def _parse_resource_locator(el):
    ...
    return {
        ...
        "description": _localised_raw(desc_els[0])
        if desc_els
        else dict(_empty_localised),
        ...
        "application-profile": _text(
            el, "cit:applicationProfile/gco:CharacterString/text()"
        ),
        "distribution-format": _texts(
            el,
...
def _parse_lineage_citation_inner(el, default_lang):
    ...
    title_raw = (
        _localised_raw(title_els[0]) if title_els else {"default": "", "local": []}
    )
    return {
        "title": _local_to_dict(title_raw, default_lang),
        "identifier": _parse_identifier(id_els[0]) if id_els else {},
        "url": _text(or_els[0], "cit:linkage/gco:CharacterString/text()")
        if or_els
        else "",
    }
...
def _parse_lineage(el, default_lang):
    ...
    for src_el in _x(el, "mrl:source/mrl:LI_Source"):
        ...
        desc_raw = (
            _localised_raw(desc_els[0]) if desc_els else {"default": "", "local": []}
        )
            ...
            json.dumps(
                {
                    ...
                    "citation": _parse_lineage_citation_inner(cit_els[0], default_lang)
                    if cit_els
                    else {},
                }
            )
    ...
    for step_el in _x(el, "mrl:processStep/mrl:LI_ProcessStep"):
        ...
        steps.append(
            json.dumps(
                {
                    "description": _local_to_dict(desc_raw, default_lang),
                    "reference": _parse_lineage_citation_inner(ref_els[0], default_lang)
                    if ref_els
                    else {},
                }
            )
...
def _infer_clean_dataset_reference_date(values):
    ...
    for date in values["dataset-reference-date"]:
        try:
            ...
            log.warning(
                "Problem converting dataset-reference-date to UTC. Defaulting to %s",
                date["value"],
            )
        dates.append(date)
    if dates:
...
def _infer_date_updated(values):
    dates = [
        d["value"] for d in values["dataset-reference-date"] if d["type"] == "revision"
    ]
    if dates:
...
def _infer_contact_email(values):
    ...
    for party in values["responsible-organisation"]:
        if (
            ...
            and isinstance(party.get("contact-info"), dict)
            and "email" in party["contact-info"]
        ):
...
def _infer_keywords(values):
    ...
    if isinstance(keywords, list):
        ...
        for item in keywords:
            ...
            if lang_dict:
                result.append(
                    {"keyword": json.dumps(lang_dict), "type": item.get("type")}
                )
...
def _infer_temporal_vertical_extent(values):
    ...
    if te_list:
        ...
        try:
            ...
            log.warning(
                "Problem converting temporal-extent dates to UTC format. "
...
def _infer_keyword_types(values):
    ...
    for kw in keywords:
        ...
        try:
            ...
            value = (
                lang_dict.get(default_lang) or next(iter(lang_dict.values()), "")
    ...
    for locator in values.get("resource-locator", []):
        for field in ("name", "description"):
            ...
            if isinstance(val, dict) and "default" in val:
                ...
                locator[field] = (
                    json.dumps(lang_dict, ensure_ascii=False) if lang_dict else ""
                )
...
def _parse_document(root):
    ...
    values["metadata-language"] = _first_text(
        root,
    ...
    values["resource-type"] = _first_texts(
        root,
    ...
    values["metadata-point-of-contact"] = [
        _parse_responsible_party_cioos(el)
        for el in _first_x(
            root,
            "mdb:contact/cit:CI_Responsibility",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='pointOfContact']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='pointOfContact']",
            _ID
            + "/mri:resourceMaintenance/mmi:MD_MaintenanceInformation/mmi:contact/cit:CI_Responsibility",
        )
    ]
    values["cited-responsible-party"] = [
        ...
        for el in _x(
            root,
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility",
        )
    ]
    values["metadata-reference-date"] = [
    ...
    values["dataset-reference-date"] = [
        _parse_reference_date(el)
        for el in _x(root, _ID + "/mri:citation/cit:CI_Citation/cit:date/cit:CI_Date")
    ]
    values["unique-resource-identifier"] = _text(
        root,
        _ID
        + "/mri:citation/cit:CI_Citation/cit:identifier/mcc:MD_Identifier/mcc:code/gco:CharacterString/text()",
    )
    ...
    values["unique-resource-identifier-full"] = [
        _parse_identifier(el)
        for el in _x(
            root, _ID + "/mri:citation/cit:CI_Citation/cit:identifier/mcc:MD_Identifier"
        )
    ]
    ...
    values["responsible-organisation"] = [
        _parse_responsible_party(el)
        for el in _first_x(
            root,
            "mdb:contact/cit:CI_Responsibility[cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='pointOfContact' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='originator' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='owner' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='rightsHolder' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='pointOfContact' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='originator' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='owner' and cit:party/cit:CI_Organisation]",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='rightsHolder' and cit:party/cit:CI_Organisation]",
        )
    ]
    values["frequency-of-update"] = _first_text(
        root,
        _ID
        + "/mri:resourceMaintenance/mmi:MD_MaintenanceInformation/mmi:maintenanceAndUpdateFrequency/mmi:MD_MaintenanceFrequencyCode/@codeListValue",
        _ID
        + "/mri:resourceMaintenance/mmi:MD_MaintenanceInformation/mmi:maintenanceAndUpdateFrequency/mmi:MD_MaintenanceFrequencyCode/text()",
    )
    values["maintenance-note"] = _text(
        root,
        _ID
        + "/mri:resourceMaintenance/mmi:MD_MaintenanceInformation/mmi:maintenanceNote/gco:CharacterString/text()",
    )
    values["progress"] = _first_texts(
        root,
        _ID + "/mri:status/mcc:MD_ProgressCode/@codeListValue",
        _ID + "/mri:status/mcc:MD_ProgressCode/text()",
    )
    values["keywords"] = [
        _parse_keyword_group(el)
        for el in _x(root, _ID + "/mri:descriptiveKeywords/mri:MD_Keywords")
    ]
    ...
    # --- Constraints ---
    values["limitations-on-public-access"] = _first_texts(
        root,
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:otherConstraints/gco:CharacterString/text()",
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:otherConstraints/gcx:Anchor/text()",
    )
    values["access-constraints"] = _first_texts(
        root,
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:accessConstraints/mco:MD_RestrictionCode/@codeListValue",
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:accessConstraints/mco:MD_RestrictionCode/text()",
    )
    values["use-constraints"] = _first_text(
        root,
        _ID
        + "/mri:resourceConstraints/mco:MD_Constraints/mco:useLimitation/gco:CharacterString/text()",
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:useLimitation/gco:CharacterString/text()",
    )
    values["use-constraints-code"] = _first_text(
        root,
        _ID
        + "/mri:resourceConstraints/mco:MD_Constraints/mco:useConstraints/mco:MD_RestrictionCode/@codeListValue",
        _ID
        + "/mri:resourceConstraints/mco:MD_Constraints/mco:useConstraints/mco:MD_RestrictionCode/text()",
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:useConstraints/mco:MD_RestrictionCode/@codeListValue",
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:useConstraints/mco:MD_RestrictionCode/text()",
    )
    values["legal-constraints-reference-code"] = _text(
        root,
        _ID
        + "/mri:resourceConstraints/mco:MD_LegalConstraints/mco:reference/cit:CI_Citation/cit:identifier/mcc:MD_Identifier/mcc:code/gco:CharacterString/text()",
    )

    # --- Aggregation / associations ---
    values["aggregation-info"] = [
        _parse_aggregation_info(el)
        for el in _x(root, _ID + "/mri:associatedResource/mri:MD_AssociatedResource")
    ]
    ...
    values["topic-category"] = _texts(
        root, _ID + "/mri:topicCategory/mri:MD_TopicCategoryCode/text()"
    )
    ...
    spatial_nodes = _x(
        root,
        _ID
        + "/mri:extent/gex:EX_Extent/gex:geographicElement/gex:EX_BoundingPolygon/gex:polygon/*",
    )
    values["spatial"] = [
    ...
    values["temporal-extent"] = [
        _parse_temporal_extent(el)
        for el in _first_x(
            root,
            _ID
            + "/mri:extent/gex:EX_Extent/gex:temporalElement/gex:EX_TemporalExtent/gex:extent/gml32:TimePeriod",
            _ID
            + "/mri:extent/gex:EX_Extent/gex:temporalElement/gex:EX_TemporalExtent/gex:extent/gml:TimePeriod",
        )
    ]
    values["vertical-extent"] = [
        _parse_vertical_extent(el)
        for el in _x(
            root,
            _ID + "/mri:extent/gex:EX_Extent/gex:verticalElement/gex:EX_VerticalExtent",
        )
    ]
    values["vertical-extent-crs"] = _texts(
        root,
        _ID
        + "/mri:extent/gex:EX_Extent/gex:verticalElement/gex:EX_VerticalExtent/gex:verticalCRSId/mrs:MD_ReferenceSystem/mrs:referenceSystemIdentifier/mcc:MD_Identifier/mcc:code/gco:CharacterString/text()",
    )
    ...
    values["browse-graphic"] = [
        _parse_browse_graphic(el)
        for el in _x(root, _ID + "/mri:graphicOverview/mcc:MD_BrowseGraphic")
    ]
    values["author"] = [
        _parse_responsible_party(el)
        for el in _first_x(
            root,
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='author']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='originator']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue ='owner']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='author']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='originator']",
            _ID
            + "/mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/text() ='owner']",
        )
    ]
...
class ISODocument:
    ...
    def __init__(self, xml_str):
        ...
        self.xml_tree = (
            self.root
        )  # ckanext-spatial base.py accesses iso_parser.xml_tree
